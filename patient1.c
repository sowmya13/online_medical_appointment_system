...
main()
{
	...
	printf("\nPhase 1:Patient 1 has TCP port number %d and IP address %s\n",(int) ntohs(client.sin_port),inet_ntoa(client.sin_addr));	
		...
		printf("\nPhase 1:Authentication request from Patient 1 with username %s and password %s has been sent to the Health Center Server",username,pwd);
		recv(s,b1,sizeof(b1),0);
		printf("\nPhase 1:Patient 1 Authentication result:%s\n",b1);
			fflush(stdout);
		printf("\nPhase 1: End of Phase 1 for Patient 1\n");	
	     ...
	     // printf("%s",b1);
	      ...
	      else
	      {
			 //printf("%s...",token);
			  ...
			  printf("\nPhase 2:The requested appointment is available and reserved to Patient1.The assigned doctor port no is %s",d.port_no);
		      close(s);
			  ...
			  fd=socket(AF_INET,SOCK_DGRAM,0);
		       ...
		       printf("\nPhase 3:Patient 1 has the dynamic port number %d and IP address %s\n",(int) ntohs(client.sin_port),inet_ntoa(client.sin_addr));
		      ...
		      recvlen = recvfrom(fd, buf,1000, 0, (struct sockaddr *)&doctor, &slen);
                if (recvlen >= 0) {
                        ...
                        printf("\nPhase 3:Patient 1 receives  %s$ estimation cost from doctor with port number %s and name %s \n",buf,d.port_no,d.name);
                        fflush(stdout);
